...
export APP_ENV=test MUSIC_PROVIDER=fake LYRICS_PROVIDER=fake WORKER_ENABLED=true MASTERING_ENABLED=false JOB_POLL_SECONDS=0.3
...
docker exec riff-postgres psql -U riff -d riff_e2e -c "TRUNCATE TABLE generations, jobs, songs, users, ai_call_telemetry CASCADE" >/dev/null
